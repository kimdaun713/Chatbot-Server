...
def create_faiss_index(embeddings):
    ...
    # GPU 텐서를 CPU로 변환하고 NumP 배열로 변환 후 추가
...
# 검색 함수: 코사인 유사도 기반 검색
def search_similar_docs(query, index, model, data, embeddings, top_k=6):
    # 검색 쿼리를 임베딩 벡터로 변환
...
# 유사한 상품 검색 예시
query = "가입 경로 인터넷"
similar_docs = search_similar_docs(query, faiss_index, model, data, embeddings, top_k=6)
